document.addEventListener('DOMContentLoaded', function() {
    ...
    // 2. "Since" yoki "For" ni tanlash mashqlari (button options)
    const sinceForQuestionsContainers = document.querySelectorAll('#practice-exercises .question .options[data-correct-answer]');
    ...
    sinceForQuestionsContainers.forEach(optionsContainer => {
        ...
        const correctAnswerString = optionsContainer.dataset.correctAnswer; // Get the correct answer from the parent div

        optionsContainer.addEventListener('click', function(event) {
            if (event.target.tagName === 'BUTTON') {
                const selectedButton = event.target;
                const selectedChoice = selectedButton.dataset.choice; // Get the chosen value from the button's data-choice
                ...
                if (isCorrect) {
                    ...
                    feedbackElement.textContent = `Noto'g'ri. To'g'ri javob: ${correctAnswerString}`;
                    feedbackElement.className = 'feedback incorrect';
    ...
    // 4. Gapni qayta tartiblash mashqlari (Drag & Drop)
    ...
    reorderQuestions.forEach(answerArea => {
        ...
        answerArea.addEventListener('drop', function(e) {
            ...
            if (draggedElement && draggedElement.parentNode === wordBank) {
                ...
                const target = e.target.closest('span') || answerArea; // Find the element being dropped on
                if (target === answerArea) {
                    answerArea.appendChild(draggedElement); // Append to end if dropped on empty area
                } else {
                    answerArea.insertBefore(draggedElement, target); // Insert before the target element
                }
            }
        });
