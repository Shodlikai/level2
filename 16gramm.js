        ...
        if (checkButton) {
            checkButton.addEventListener('click', function() {
                const userAnswer = Array.from(answerArea.children)
                                    ...
                                    .join(' ')
                                    .replace(/\.$/, '').replace(/\?$/, ''); // Remove trailing dot/question mark for comparison
                const correctAnswer = answerArea.dataset.answer.trim().replace(/\.$/, '').replace(/\?$/, ''); // Remove trailing dot/question mark from correct answer
